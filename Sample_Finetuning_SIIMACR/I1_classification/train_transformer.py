...
def train(model, data_loader, optimizer, criterion, epoch, warmup_steps, device, scheduler, args,config,writer):
    ...
    for i, sample in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        loss = criterion(pred_class,label.unsqueeze(1))
        loss.backward()
        optimizer.step()    
...
def valid(model, data_loader, criterion,epoch,device,config,writer):
    ...
    for i, sample in enumerate(data_loader):
        ...
        with torch.no_grad():
            ...
            val_loss = criterion(pred_class,label.unsqueeze(1))
...
def main(args, config):
    ...
    train_dataset = RSNA_Dataset(rsna, train_indices, is_train = True, undersample=False) 
    ...
    val_dataset = RSNA_Dataset(rsna, val_indices, is_train = False, undersample=False) 
