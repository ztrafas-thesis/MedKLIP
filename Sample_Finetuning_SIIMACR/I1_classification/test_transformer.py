...
def test(args,config):
    ...
    num_samples = int(len(indices) * 0.1)  # Calculate 10% of the total indices
    test_indices = indices[:num_samples]
    np.random.shuffle(indices)

    test_dataset = RSNA_Dataset(rsna, test_indices, is_train = False, undersample=False) 
    ...
    for i, sample in enumerate(test_dataloader):
        ...
        gt = torch.cat((gt, label.unsqueeze(1)), 0)
